...
def fetch_all_receipts(committee_ids, active_cycles, member_name):
    ...
    for cmte in committee_ids:
        ...
        # grassroots   : sum of per-cycle `individual_unitemized_contributions`
        #                — the FEC's own authoritative small-dollar figure
        #                (contributions of $200 or under, which campaigns
        #                report as a single unitemized total and are NOT
        #                required to itemize on Schedule A).
        #
        ...
        if t and t.get("results"):
            for row in t["results"]:
                ...
                grassroots   += row.get("individual_unitemized_contributions") or 0
        time.sleep(0.1)
