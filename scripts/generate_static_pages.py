...
def render_donations(profile: dict, fec_record: dict) -> str:
    ...
    # total_raised from FEC may be an int, a numeric string, or an empty string
    # (some records carry ''). Only render the FEC row when it's a real number;
    # otherwise fall back to the profile value, exactly as if FEC had no key.
    fec_tr = fec_record.get("total_raised") if "total_raised" in fec_record else None
    try:
        fec_tr_num = int(float(fec_tr)) if fec_tr not in (None, "") else None
    except (TypeError, ValueError):
        fec_tr_num = None
    if fec_tr_num is not None:
        row("Total raised (career)", f"${fec_tr_num:,}", "FEC")
    ...
    if pending_member and donations.get("corporate_total") and not fec_record.get("special_interest_total"):
        ...
        _sit = fec_record["special_interest_total"]
        try:
            _sit_num = int(float(_sit)) if _sit not in (None, "") else None
        except (TypeError, ValueError):
            _sit_num = None
        if _sit_num is not None:
            row("Corporate / special interest total", f"${_sit_num:,}", "FEC + OpenSecrets")
    ...
    for label, prof_key, fec_key in sectors:
        ...
        if fec_key in fec_record:
            ...
            try:
                v_num = int(float(v)) if v not in (None, "") else None
            except (TypeError, ValueError):
                v_num = None
            if v_num is not None:
                row(label, f"${v_num:,}", "FEC + OpenSecrets")
            elif prof_val:
                row(label, prof_val)
        elif prof_val:
            row(label, prof_val)
