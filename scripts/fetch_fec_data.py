        ...
        try:
            with open(fpath, encoding='utf-8') as f:
                for row in _csv.DictReader(f):
                    ...
                    if not raw or amt == 0:
                        continue
                    # Build a normalized token set for matching
                    # Strip party suffix, normalize hyphens as spaces
                    import re as _re
                    clean = _re.sub(r'\s*\([RDI][^\)]*\)\s*$', '', raw).strip()
                    key   = _normalize_oil(clean)
                    state_key = _normalize_oil(state)
                    # Store as (amount, state_key) — matched by name tokens + state
                    if key not in OIL_DATA or amt > OIL_DATA[key][0]:
                        OIL_DATA[key] = (amt, state_key)
                    total += 1
        except FileNotFoundError:
...
def _best_csv_match(member_name, member_state, csv_data):
    """
    Token-set match a member to a row in `csv_data` (dict of csv_key -> (amount, state_key)).
    
    Returns the dollar amount of the best match, or 0 if no safe match found.
    
    SAFETY RULE: requires at least 2 NAME-token overlaps (e.g., first AND last name).
    Surname-only matches are rejected even if state matches. This prevents
    parent/child or junior/senior collisions like Adelita Grijalva vs her
    father Raul Grijalva (both AZ-07, surname "Grijalva" overlap = 1 token,
    state match would have boosted to score 3 = false positive).
    
    State match is used only as a tiebreaker between candidates that already
    pass the 2-name-token threshold.
    """
    if not csv_data:
        return 0
    ...
    for csv_key, (amt, csv_state) in csv_data.items():
        csv_tokens = {t for t in csv_key.split() if len(t) > 1}
        overlap    = name_tokens & csv_tokens
        # SAFETY: require at least 2 name tokens to overlap.
        # A surname-only match (overlap=1) is never enough, no matter the state.
        if len(overlap) < 2:
            continue
        score = len(overlap)
        # State match is now a tiebreaker, not a way to promote a 1-token match.
        if csv_state and norm_state and csv_state in norm_state:
            score += 2
        if score > best_score:
            best_score = score
            best_amt   = amt
        ...
        try:
            with open(fpath, encoding='utf-8') as f:
                for row in _csv.DictReader(f):
                    ...
                    key       = _normalize_oil(clean)
                    state_key = _normalize_oil(state)
                    if key not in PHARMA_DATA or amt > PHARMA_DATA[key][0]:
                        PHARMA_DATA[key] = (amt, state_key)
                    total += 1
        except FileNotFoundError:
        ...
        try:
            with open(fpath, encoding='utf-8') as f:
                for row in _csv.DictReader(f):
                    ...
                    key       = _normalize_oil(clean)
                    state_key = _normalize_oil(state)
                    if key not in DEFENSE_DATA or amt > DEFENSE_DATA[key][0]:
                        DEFENSE_DATA[key] = (amt, state_key)
                    total += 1
        except FileNotFoundError:
        ...
        try:
            with open(fpath, encoding='utf-8') as f:
                for row in _csv.DictReader(f):
                    ...
                    key       = _normalize_oil(clean)
                    state_key = _normalize_oil(state)
                    if key not in FINANCE_DATA or amt > FINANCE_DATA[key][0]:
                        FINANCE_DATA[key] = (amt, state_key)
                    total += 1
        except FileNotFoundError:
        ...
        try:
            with open(fpath, encoding='utf-8') as f:
                for row in _csv.DictReader(f):
                    ...
                    key       = _normalize_oil(clean)
                    state_key = _normalize_oil(state)
                    if key not in TECH_DATA or amt > TECH_DATA[key][0]:
                        TECH_DATA[key] = (amt, state_key)
                    total += 1
        except FileNotFoundError:
...
def get_aipac_from_trackaipac(name):
    ...
    Uses normalized name matching:
      ...
      2. Token-set match requiring at least 2 name tokens to overlap
         (first AND last name, or equivalent). Surname-only matches are
         REJECTED to prevent same-surname collisions like Thomas Massie (KY-04)
         being conflated with a Massachusetts Senate candidate of the same
         surname, or Adelita Grijalva (AZ-07) being conflated with her father.
    ...
    # 2. SAFE fuzzy match — require 2+ name tokens to overlap.
    #    This prevents the surname-only collision bug where e.g. "thomas massie"
    #    would otherwise fall through to the only "massie" entry in the file
    #    (a different person entirely), attributing wrong dollar amounts.
    name_tokens = {t for t in key.split() if len(t) > 1}
    if len(name_tokens) < 2:
        # Member's own name has fewer than 2 usable tokens — can't safely match.
        return empty
    ...
    for k, v in TRACKAIPAC_DATA.items():
        ...
        if overlap > best_overlap:
            ...
            # Two different entries tied at the same overlap — ambiguous,
            # refuse to guess rather than risk picking the wrong one.
            ambiguous = True
