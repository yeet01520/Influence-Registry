...
def resolve_all_candidate_ids(name, state, current_office):
    ...
    ALL offices they have ever held (House and/or Senate).
...
def get_aipac_from_trackaipac(name):
    ...
    Uses normalized name matching: exact first, then last-name fuzzy fallback.
    """
    def _norm(n):
        n = n.lower()
    ...
    # 2. Last-name fuzzy — find all entries sharing the last token
    parts = key.split()
    if not parts:
        return empty
    last = parts[-1]
    candidates = {k: v for k, v in TRACKAIPAC_DATA.items() if k.split()[-1] == last}
    if len(candidates) == 1:
