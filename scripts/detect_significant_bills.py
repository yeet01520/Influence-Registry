...
STATS = {
    ...
    "senate_outside_lookback": 0,
    "senate_menu_skip_noise": 0,
}
...
def build_draft_from_house_vote(vote, congress, session, api_key):
    ...
    # Independents (party == "I") are excluded — they don't belong to either
    # party and shouldn't be labeled "R-side" or "D-side" defectors.
    # They still appear in yes_votes / no_votes for the full roll-call record.
    PARTISAN = {"R", "D"}
    defectors_yes = []
    defectors_no = []
    party_counts_yes = {}
    ...
    if majority_party_yes:
        for m in yes_votes:
            if m["party"] in PARTISAN and m["party"] != majority_party_yes:
                defectors_yes.append(m["name"])
    if majority_party_no:
        for m in no_votes:
            if m["party"] in PARTISAN and m["party"] != majority_party_no:
...
def build_draft_from_senate_vote(vote_detail, vote_date_iso, congress):
    ...
    # Defectors: only flag true party crossovers (R or D).
    # Independents (I) caucus with one side by convention but are not party
    # members — labeling them "R-side" or "D-side" defectors is misleading.
    PARTISAN = {"R", "D"}
    defectors_yes = [m["name"] for m in yes_votes
                     if m["party"] in PARTISAN and m["party"] != majority_yes][:5]
    defectors_no = [m["name"] for m in no_votes
                    if m["party"] in PARTISAN and m["party"] != majority_no][:5]
    defector_hint = ""
    if defectors_yes or defectors_no:
        parts = []
        if defectors_yes:
...
def process_senate_votes(congress, session, since_date, existing_ids, existing_draft_ids):
    ...
    for vote in menu:
        ...
        if not vote_date_iso or vote_date_iso < since_date:
            STATS["senate_outside_lookback"] += 1
        ...
        if "cloture" in question or "nomination" in question:
            STATS["senate_menu_skip_noise"] += 1
            continue
...
def main():
    ...
    print(f"    ↳ outside lookback:   {STATS['senate_outside_lookback']}", flush=True)
    print(f"    ↳ cloture/nomination: {STATS['senate_menu_skip_noise']}", flush=True)
