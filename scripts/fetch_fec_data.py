        ...
        if t and t.get("results"):
            for row in t["results"]:
                ...
                if not cyc or cyc not in active or cyc in seen_cycles:
                    continue
                seen_cycles.add(cyc)
                receipts = row.get("receipts") or 0
                if receipts:
                    cmte_cycles.add(cyc)
                total_raised += receipts
        time.sleep(0.1)

        # ── 2. Grassroots: itemized <=$200 bucket, FEC-computed/deduped ─
        # (includes unitemized small-dollar money). One call per active
        # cycle for this committee. Do NOT add unitemized again from the
        # totals endpoint — by_size already contains it.
        for cycle in sorted(cmte_cycles, reverse=True):
            bs = get("/schedules/schedule_a/by_size/", {
                "committee_id": cmte,
                "cycle": cycle,
                "size": 0,            # the $200-and-under bucket
                "per_page": 100,
            })
            if bs and bs.get("results"):
                for row in bs["results"]:
                    grassroots += row.get("total") or 0
            time.sleep(0.1)

        # ── 3. NRA / tracked-PAC money: targeted, not paginated ─────────
        # Instead of paginating ALL of a committee's PAC receipts to find the
        # occasional tracked-PAC check (which cost 100K+ calls for senior
        # members), query Schedule A filtered to EACH tracked source committee
        # directly. The recipient's own committee is the receiving end, so we
        # ask: "what did source PAC X give to committee {cmte}?" These return
        # at most a handful of rows. Total calls per member = (committees) x
        # (tracked source PACs) — bounded and tiny.
        #
        # Every sector except those in SECTOR_CMTE_IDS comes from OpenSecrets
        # CSVs and is overwritten after this function returns. Today
        # SECTOR_CMTE_IDS holds only NRA-family PACs.
        for src_cmte, sector in _TRACKED_SOURCE_PACS:
            page = 1
            while True:
                data = get("/schedules/schedule_a/", {
                    "committee_id": cmte,                 # recipient
                    "contributor_committee_id": src_cmte, # tracked source PAC
                    "per_page": 100,
                    "page": page,
                })
                if not data or not data.get("results"):
                ...
                if not pg.get("pages") or page >= pg["pages"]:
                    break
                page += 1
                time.sleep(0.1)
            time.sleep(0.05)
