...
─────────────────────────────────────────────────────────────────────────────
1. FEC API key: y6TD120jyChOZFHcsvbAaczqs2TOHh6sgVq7MF1W
...
def get_oil_amount(member_name, member_state):
    ...
    Uses token-set overlap matching to handle compound last names,
    middle names/initials, and hyphenated names.
    ...
    for csv_key, (amt, csv_state) in OIL_DATA.items():
        csv_tokens = {t for t in csv_key.split() if len(t) > 1}
        overlap    = name_tokens & csv_tokens
        score      = len(overlap)
        # State match adds a strong tiebreaker
...
def get_pharma_amount(member_name, member_state):
    ...
    for csv_key, (amt, csv_state) in PHARMA_DATA.items():
        csv_tokens = {t for t in csv_key.split() if len(t) > 1}
        overlap    = name_tokens & csv_tokens
        score      = len(overlap)
        if csv_state and norm_state and csv_state in norm_state:
...
def get_defense_amount(member_name, member_state):
    ...
    for csv_key, (amt, csv_state) in DEFENSE_DATA.items():
        csv_tokens = {t for t in csv_key.split() if len(t) > 1}
        overlap    = name_tokens & csv_tokens
        score      = len(overlap)
        if csv_state and norm_state and csv_state in norm_state:
...
def get_finance_amount(member_name, member_state):
    ...
    for csv_key, (amt, csv_state) in FINANCE_DATA.items():
        csv_tokens = {t for t in csv_key.split() if len(t) > 1}
        overlap    = name_tokens & csv_tokens
        score      = len(overlap)
        if csv_state and norm_state and csv_state in norm_state:
...
def get_tech_amount(member_name, member_state):
    ...
    for csv_key, (amt, csv_state) in TECH_DATA.items():
        csv_tokens = {t for t in csv_key.split() if len(t) > 1}
        overlap    = name_tokens & csv_tokens
        score      = len(overlap)
        if csv_state and norm_state and csv_state in norm_state:
