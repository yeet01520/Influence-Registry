...
# Loaded from scripts/skip_members.json so the same list is also readable by
# the workflow YAML (without resorting to fragile regex parsing of Python).
# To add or remove a skipped member, edit that JSON file.
SKIP_MEMBERS_PATH = os.path.join(SCRIPT_DIR, "skip_members.json")
SKIP_MEMBERS = set()
if os.path.exists(SKIP_MEMBERS_PATH):
    try:
        with open(SKIP_MEMBERS_PATH) as _f:
            _sm = json.load(_f)
        SKIP_MEMBERS = set(_sm.get("skip", []))
        if SKIP_MEMBERS:
            print(f"Loaded SKIP_MEMBERS ({len(SKIP_MEMBERS)}): {sorted(SKIP_MEMBERS)}")
    except Exception as _e:
        print(f"⚠ Could not parse {SKIP_MEMBERS_PATH}: {_e}")
        ...
        if entry.get("aipac", 0) == 0:
            for variant in variants:
                ...
                if ta and ta.get("total", 0) > 0:
                    ...
                    # aipac_pacs + aipac_ie. aipac_lobby_donors is preserved
                    # in the record but excluded from the headline aipac total
                    # per the TrackAIPAC-consistent methodology.
                    entry["aipac_pacs"]         = ta["pacs"]
                    entry["aipac_ie"]           = ta["ie"]
                    entry["aipac_lobby_donors"] = ta["lobby_donors"]
                    entry["aipac"]              = ta["pacs"] + ta["ie"]
...
# ── Enforce AIPAC methodology on every record ───────────────────────────────
# Defense in depth: fetch_fec_data.py historically wrote
#   aipac = aipac_pacs + aipac_ie + aipac_lobby_donors
# We've since moved to the TrackAIPAC-consistent definition:
#   aipac = aipac_pacs + aipac_ie  (lobby donors preserved but not summed)
# Rather than rely on v7 being kept in sync, we normalize every record here
# unconditionally. This also recomputes special_interest_total so the H3
# validator invariant holds. Safe to run repeatedly: idempotent.
normalized = 0
for name, entry in final_data.items():
    if name == "_meta" or not isinstance(entry, dict):
        continue
    pacs = entry.get("aipac_pacs", 0) or 0
    ie   = entry.get("aipac_ie", 0) or 0
    new_aipac = pacs + ie
    if entry.get("aipac") != new_aipac:
        entry["aipac"] = new_aipac
        normalized += 1
    # Recompute SIT against the methodology-correct aipac plus other sectors.
    # Matches the validator's H3 formula (pacs + ie + other sector totals).
    entry["special_interest_total"] = (
        pacs + ie +
        (entry.get("fossil_fuels", 0) or 0) +
        (entry.get("pharma",       0) or 0) +
        (entry.get("defense",      0) or 0) +
        (entry.get("finance",      0) or 0) +
        (entry.get("tech",         0) or 0) +
        (entry.get("nra",          0) or 0)
    )
if normalized:
    print(f"\nNormalized aipac field on {normalized} records to pacs+ie methodology")
