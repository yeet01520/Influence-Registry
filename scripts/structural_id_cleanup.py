...
def main():
    ...
    # Note: We do NOT read or merge existing members_to_refetch.json.
    # This script writes a fresh, minimal refetch list containing only members
    # whose primary actually changed. The audit run's refetched 285 already have
    # correct outside spending data — re-running them now would be wasteful.
    ...
    # Build FRESH refetch list — only members whose primary actually changed.
    # Members with cleaned contamination but unchanged primary don't need refetch:
    # their Schedule E aggregation will naturally exclude the dropped IDs on the next
    # quarterly auto-refresh. Refetching them now would just re-run identical queries.
    refetch_set = set()
