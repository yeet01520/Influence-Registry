...
def probe_itemised_endpoint(cid):
    ...
    for path, params in candidates:
        ...
        rows = (data or {}).get("results") or []
        for r in rows:
            name = r.get("contributor_name") or r.get("donor_name")
            amt = r.get("contribution_receipt_amount") or r.get("donation_amount")
            if name and amt:
                print(f"  Itemised donations: using {path} "
                      f"(probe returned '{name}' for ${amt:,.0f})", flush=True)
                return path, {k: v for k, v in params.items() if k != "per_page"}
    print("  WARNING: no endpoint returned itemised inaugural donations. "
          "Writing the committee-level total only. The donor list is filed on "
          "FEC Form 13 and may need to be pulled from docquery.fec.gov instead.",
          flush=True)
    return None, None
