...
def check_grassroots_corporate_total(d):
    """Grassroots members must show $0 / '$0 corporate PAC' / similar zero label —
    not a real dollar figure."""
    print("\n[9] Grassroots corporate_total labels...")
    ...
    for name in grassroots:
        ...
        ct = prof.get("donations", {}).get("corporate_total", "")
        if not ct: continue
        # Acceptable if it contains "$0" as a label (e.g. "$0 corporate PAC")
        if "$0" in ct:
            continue
        val = parse_money_str(ct)
        if val is not None and val > 0:
            err(f"Grassroots corporate_total not zero — {name}: '{ct}' parses to ${val:,.0f}")
            issues += 1
    if issues == 0:
        print(f"  ✅ All {len(grassroots)} grassroots members show $0 corporate PAC")
    else:
        print(f"  ❌ {issues} grassroots members have non-zero corporate_total")
...
def main():
    ...
    check_grassroots_corporate_total(d)
