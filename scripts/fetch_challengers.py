...
def former_member_bioguides(challengers):
    """
    Give photos to challengers who used to serve.

    Photos come from the Bioguide CDN keyed on a bioguide ID, and challengers
    have none, so all 135 rendered as initials. Former members do have one, and
    the historical legislator file carries it.

    The match REQUIRES the state to agree. Matching on name alone tied a
    Florida challenger named Chris Collins to the New York representative of
    the same name, and a Minnesota one named Jacob Johnson to a Utah member who
    left office in 1915.

    Returns {name: bioguide}.
    """
    people = []
    for url in (HISTORICAL_URL, CURRENT_URL):
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "InfluenceRegistry/1.0"})
            with urllib.request.urlopen(req, timeout=120) as r:
                people.extend(json.loads(r.read()))
        except Exception as e:
            print(f"  WARNING: could not fetch {url.rsplit('/', 1)[-1]} ({e})")
    idx = {}
    for m in people:
        n = m.get("name", {})
        t = (m.get("terms") or [{}])[-1]
        for cand in (n.get("official_full"),
            ...
            if cand:
                idx.setdefault(_norm(cand), []).append((m["id"]["bioguide"], t.get("state")))
    out = {}
    for c in challengers:
        for b, st in idx.get(_norm(c.get("name", "")), []):
            if st and st == c.get("state"):
                out[c["name"]] = b
                break
    return out
...
def main():
    ...
    # Photos for anyone who previously served.
    bio_path = DATA / "bioguide.json"
    if bio_path.exists():
        bio = json.loads(bio_path.read_text())
        found = former_member_bioguides(out)
        newbio = {k: v for k, v in found.items() if k not in bio}
        if newbio:
            bio.update(newbio)
            bio_path.write_text(json.dumps(bio, indent=2, ensure_ascii=False))
        print(f"\nBioguide photos matched for former members: {len(found)} "
              f"({len(newbio)} newly added)")
        print(f"Challengers with no photo source: {sum(1 for c in out if c['name'] not in bio)} "
              f"of {len(out)}. Never-served candidates have no official portrait; "
              f"they render as initials unless a photo is added to "
              f"data/photo_overrides.json.")
