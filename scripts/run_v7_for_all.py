...
# ── Build members.json from canonical house.json + senate.json ──────────────
# The v7 fetcher expects a list of {"name", "state", "office"} dicts where
# office is "S" for Senate or "H" for House. We generate this on the fly
# from the canonical chamber files so there's a single source of truth.
generated_members = False
if not os.path.exists(TARGET_MEMBERS):
    if not os.path.exists(HOUSE_JSON):
        sys.exit(f"ERROR: {HOUSE_JSON} not found — can't build members.json")
    if not os.path.exists(SENATE_JSON):
        sys.exit(f"ERROR: {SENATE_JSON} not found — can't build members.json")

    print(f"members.json not present — building from house.json + senate.json")
    ...
    print(f"Using existing members.json at {TARGET_MEMBERS}")
...
try:
    ...
    if count < 100:
        print(f"  ⚠ WARNING: only {count} members. Expected ~535 for full Congress.")
...
try:
    ...
    # ── Remove the members.json we generated so it doesn't get committed ───
    # If users had their own members.json, we kept it (generated_members=False).
    if generated_members and os.path.exists(TARGET_MEMBERS):
        try:
            os.remove(TARGET_MEMBERS)
            print("Cleaned up generated members.json")
        except Exception:
            pass
