...
# Members appointed to their seat who have never run a federal campaign, so no
# FEC campaign-finance history exists. Their score is reported as N/A rather than
# a misleading "Clean" 0-15%, mirroring NO_CAMPAIGN_NAMES in index.html.
# Do NOT add special-election winners here — they ran campaigns; their missing
# data is a fetch issue to fix, not a reason to mask. Remove a name once the
# member runs for federal office and acquires real FEC data.
CHALLENGER_NAMES = set()  # populated in main() from challengers.json

NO_CAMPAIGN_NAMES = {
    "Jon Husted", "Alan Armstrong", "Ashley Moody",
}


# ─── Helpers (mirrors helpers in index.html) ────────────────────────────────────

# Members where profiles.json records AIPAC money that fec.json does not.
# Collected during scoring and reported at the end rather than silently
# dropped, because the profile figure may be independent expenditure OR a
# gap in the FEC pull, and those need different fixes.
AIPAC_SOURCE_CONFLICTS = []

# Independent expenditure by member, loaded in main().
OUTSIDE_LOOKUP = {}


def funding_score(fec_record):
    """
    How much of a member's CONTRIBUTIONS came from PACs.

        pac   = pac_total
        denom = individual_total + pac_total + party_total
        score = pac / denom * 150, capped at 100

    Party committee money stays in the DENOMINATOR, because it is money they
    received, but not in the numerator. It is different in kind: the NRCC
    funding one of its own races is not an interest buying access. It is also
    negligible, $15.1M against $2.45B in PAC money across the roster, so 433 of
    536 members scored identically either way and only 2 differed by more than
    four points. Blending it in cost a sentence of justification for a rounding
    error.

    Replaces (total_raised - grassroots), which was standing in for PAC money
    and was not PAC money. That gap also holds candidate loans, transfers and
    refunds, so Rick Scott scored 100 on an 8% PAC share: $103.5M of his own
    self-funding was being counted as organised money. Using FEC's own PAC and
    party fields measures the thing the score claims to measure.

    AIPAC PAC contributions are already inside pac_total, so they count exactly
    like every other PAC and are not added again.
    """
    ind = fec_record.get("individual_total")
    pac = fec_record.get("pac_total")
    party = fec_record.get("party_total") or 0
    if not isinstance(ind, (int, float)) or not isinstance(pac, (int, float)):
        return None
    denom = ind + pac + party
    if denom <= 0:
        return None
    return int(round(min(pac / denom * 150, 100)))


def outside_score(name, fec_record):
    """
    How much was spent to ELECT them by groups they cannot legally coordinate
    with, as a share of what their own campaign raised.

        score = ie_supporting / total_raised * 100, capped at 100

    So 100 means outside groups spent at least as much as the campaign did,
    the same threshold the outside-spending chart already marks.
    ...
    Opposition spending is shown separately on the member's profile.

    Every committee counts the same way: AIPAC's UDP, NRA Political Victory
    Fund, LCV Victory Fund. The old formula added pro-Israel spending to the
    funding score and no one else's, which was a data-availability artifact
    rather than a principle.
    """
    tr = fec_record.get("total_raised")
    if not isinstance(tr, (int, float)) or tr <= 0:
        return None
    rec = OUTSIDE_LOOKUP.get(name) or {}
    ies = rec.get("total_supporting") or 0
    return int(round(min(ies / tr * 100, 100)))


def band(pct):
    if pct >= 80: return "High Risk", "#b91c1c"
    if pct >= 60: return "Moderate Risk", "#ea580c"
    if pct >= 40: return "Some Corporate", "#d97706"
    if pct >= 20: return "Low Risk", "#ca8a04"
    return "Clean", "#16a34a"
...
def calc_score(name: str, profile: dict, merged_donations: dict,
               grassroots_names: set, aipac_lookup: dict, sector_lookups: dict,
               fec_record: dict = None) -> dict:
    """
    ...
    """
    # Branch 0: No federal campaign history (appointed members) — report N/A.
    # Takes precedence over all other branches so an AIPAC-only or zero total
    # never produces a misleading "Clean" label for someone never on a ballot.
    if name in NO_CAMPAIGN_NAMES:
        return {
            "pct": 0, "lbl": "N/A", "vto": False, "col": "#9ca3af",
            "super_pac_total": 0, "any_corporate": False, "reasons": [],
            "no_campaign": True,
            "score_basis": "No campaign data. Appointed member with no federal campaign finance history on file.",
        }

    # Branch 0b: Challengers — RETIRED manual baseline. Challengers are now
    # scored by the standard B2 funding-mix formula below, the same as everyone
    # else, so an individual-funded challenger lands at its true computed value
    # rather than a hand-set floor. (Cabinet/SCOTUS keep manual scores in
    # Branch 2 because they have no campaign finance data to compute from.)

    # Branch 1: Grassroots members — RETIRED static baseline. B2 already scores
    # individual-funded members low, so the protective baseline is no longer
    # needed. Grassroots members now fall through to B2 and are scored on their
    # actual funding mix.
    ...
    # AIPAC: use only money the CAMPAIGN RECEIVED, not independent expenditure.
    #
    # The blended `aipac` field is aipac_pacs + aipac_ie. Folding the whole
    # thing into a share-of-receipts formula treated money the campaign never
    # touched as though it had been received, and the arithmetic broke: George
    # Latimer's AIPAC figure is $19.2M against $7.2M in total receipts, which
    # scored him 100 on a real PAC share of 6%. Sarah Elfreth scored 98 on
    # $20,892 of AIPAC money actually received; Maxine Dexter scored 85 on
    # nothing at all.
    #
    # No other group's independent expenditure was ever added this way, so the
    # rule was also inconsistent: the same file records NRA Political Victory
    # Fund and LCV Victory Fund spending that the score ignored.
    #
    # Independent expenditure has not disappeared from the site. It is shown in
    # the outside-spending chart and on member profiles, where every group is
    # measured the same way. It just no longer inflates a score that claims to
    # measure how a campaign was funded.
    aipac_received = fec_record.get("aipac_pacs") if fec_record else None
    profile_aipac = parse_dollar(d.get("aipac"))
    if isinstance(aipac_received, (int, float)):
        aipac_amt = float(aipac_received)
        if profile_aipac > 0 and aipac_amt == 0:
            AIPAC_SOURCE_CONFLICTS.append((name, profile_aipac))
    else:
        # Older records without the split: fall back to the profile value, which
        # predates the IE data and is contributions only.
        aipac_amt = parse_dollar(d.get("aipac"))
    ...
    # ─── Individual-share formula (v3 / "B2") ─────────────────────────
    # Score reflects the FRACTION of money that is NON-individual, not the raw
    # dollar amount of sector money. AIPAC is folded in as fully non-individual
    # PAC money. Other sectors are already inside total_raised/grassroots and
    # are surfaced via the individual-vs-PAC split, not added as raw dollars.
    #   non_individual = (total_raised - grassroots) + AIPAC CONTRIBUTIONS
    #   denom          = total_raised + AIPAC CONTRIBUTIONS
    #   score          = (non_individual / denom) * 150, capped at 100
    #
    # AIPAC here means aipac_pacs, money the campaign received. Independent
    # expenditure is excluded; see the note above aipac_amt.
    fr = fec_record or {}
    total_raised = float(fr.get("total_raised") or 0)
    grassroots   = float(fr.get("grassroots") or 0)

    if total_raised <= 0:
        # No FEC committee totals (e.g. a member who moved to an office that
        # doesn't file, like a sitting VP). We can't compute a funding-mix
        # share. Fall back to: (a) a manual corruption_score if set, else
        # (b) flag any confirmed AIPAC/PAC money rather than returning a
        # misleading 0 for someone with real special-interest money.
        if profile.get("corruption_score") is not None:
            mb = int(profile["corruption_score"])
            col = "#16a34a" if mb < 20 else "#ca8a04" if mb < 40 else "#d97706" if mb < 60 else "#ea580c" if mb < 80 else "#b91c1c"
            lbl = "Clean" if mb < 20 else "Low Risk" if mb < 40 else "Some Corporate" if mb < 60 else "Moderate Risk" if mb < 80 else "High Risk"
            return {
                "pct": mb, "lbl": lbl, "vto": False, "col": col,
                "super_pac_total": int(super_pac_total), "any_corporate": any_corporate,
                "reasons": reasons,
                "score_basis": "No FEC committee totals; manual editorial score based on record.",
            }
        if aipac_amt > 0:
            # Confirmed PAC money but no totals — score on AIPAC presence so it
            # isn't reported as clean. Scaled by amount, capped mid-range.
            mb = min(60, 30 + int(aipac_amt / 100_000))
            return {
                "pct": mb, "lbl": "Some Corporate", "vto": False, "col": "#d97706",
                "super_pac_total": int(super_pac_total), "any_corporate": True,
                "reasons": reasons or ["AIPAC"],
                "score_basis": "No FEC committee totals; scored on confirmed AIPAC/PAC money.",
            }
        return {
            "pct": 0, "lbl": "Clean", "vto": False, "col": "#16a34a",
            "super_pac_total": int(super_pac_total), "any_corporate": any_corporate,
            "reasons": reasons,
            "score_basis": "No FEC fundraising totals available to score funding mix.",
        }

    # ── TWO SCORES ────────────────────────────────────────────────────
    # They answer different questions and collapsing them into one number lost
    # both. Funding = who paid the campaign. Outside = who paid to put them in
    # the seat without going through the campaign at all.
    fscore = funding_score(fr)
    if fscore is None:
        # No FEC contribution split on file: fall back to the old receipts-gap
        # measure so the member is still scored rather than blank.
        non_individual = max(total_raised - grassroots, 0) + aipac_amt
        denom = total_raised + aipac_amt
        fscore = int(round(min(((non_individual / denom) if denom > 0 else 0) * 150, 100)))
    oscore = outside_score(name, fr)
    if oscore is None:
        oscore = 0

    score = fscore
    lbl, col = band(fscore)
    olbl, ocol = band(oscore)

    return {
        "pct": score, "lbl": lbl, "vto": False, "col": col,
        # `pct` stays the FUNDING score so every existing consumer keeps
        # working. The outside score is additive.
        "funding_pct": fscore, "funding_lbl": lbl, "funding_col": col,
        "outside_pct": oscore, "outside_lbl": olbl, "outside_col": ocol,
...
def main():
    ...
    # Independent expenditure, used for the SECOND score. Optional so a missing
    # file degrades to funding-only rather than failing the run.
    try:
        outside_lookup = json.load(open(data_dir / "outside_spending.json"))
    except FileNotFoundError:
        print("  WARNING: outside_spending.json missing; outside scores will be 0")
        outside_lookup = {}
    global OUTSIDE_LOOKUP
    OUTSIDE_LOOKUP = outside_lookup

    grassroots_names = set(tags.get("grassroots") or [])
    global CHALLENGER_NAMES
    try:
        _chal = json.load(open(data_dir / "challengers.json"))
        CHALLENGER_NAMES = set(m["name"] for m in _chal)
    except FileNotFoundError:
    ...
    if AIPAC_SOURCE_CONFLICTS:
        print(f"\n  AIPAC source conflicts ({len(AIPAC_SOURCE_CONFLICTS)}): profiles.json "
              f"records money that fec.json does not. Scored on the FEC figure "
              f"(zero); check whether the profile value is independent expenditure "
              f"or a gap in the FEC pull.", file=sys.stderr)
        for nm, amt in sorted(AIPAC_SOURCE_CONFLICTS, key=lambda x: -x[1])[:12]:
            print(f"      {nm:26s} profile ${amt:>12,.0f}   fec $0", file=sys.stderr)
    return 0
