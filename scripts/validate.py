...
def check_aipac_vs_fec_pacs(d):
    """[S1] AIPAC_DATA (legacy: full Israel Lobby Total per TrackAIPAC, i.e.
    PACs + IE + Lobby Donors) compared against fec.aipac, which under the
    current methodology is PACs + IE only. Drift here is now expected and
    informational: the gap roughly equals aipac_lobby_donors for each
    member. The check stays in place so changes to either source are still
    surfaced, but warnings should be interpreted with the methodology shift
    in mind."""
