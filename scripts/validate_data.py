...
ERRORS (arithmetically impossible; exit 1):
  ...
  - a roster member who is no longer in congress-legislators, or a sitting
    member missing from the roster. David Scott stayed in the roster for months
    after his death in April 2026 because the member COUNT still looked right:
    two members had died and two had replaced them, netting to 537.
...
def roster_drift(senate, house, bioguide):
    """
    Compare the site roster against the official congress-legislators file,
    in BOTH directions.

    This check exists because David Scott died on 22 April 2026 and stayed in
    the roster for months. Nothing caught it: he still had a card, a rank, and
    a line in every chamber median. What surfaced it was a missing photo,
    because a dead member has no bioguide entry and therefore no portrait.

    Lindsey Graham had died too, and the two members who replaced them, Darline
    Graham and James Gallagher, were missing entirely. The counts netted out at
    537, which is exactly why nothing looked wrong.

    Returns (departed, missing) as lists of (name, chamber, seat).
    """
    try:
        ...
        print(f"  WARNING: could not fetch the official legislator file ({e}); "
              f"roster drift not checked.")
        return None, None

    by_bio = {m["id"]["bioguide"]: m for m in leg if m.get("id", {}).get("bioguide")}
    official_names = set()
    for m in leg:
        n = m["name"]
        for cand in (n.get("official_full"),
                     (n.get("first", "") + " " + n.get("last", "")).strip(),
                     (n.get("nickname", "") + " " + n.get("last", "")).strip()):
            if cand:
                official_names.add(_norm(cand))

    departed = []
    roster_bio, roster_names = set(), set()
    for m in senate + house:
        nm = m["name"]
        b = bioguide.get(nm)
        if b:
            roster_bio.add(b)
        roster_names.add(_norm(nm))
        if b and b in by_bio:
            continue
        if _norm(nm) in official_names:
            continue
        ch = m.get("chamber") or ("Senate" if m in senate else "House")
        departed.append((nm, ch, m.get("district") or m.get("state") or ""))

    missing = []
    for m in leg:
        n = m["name"]
        b = m["id"]["bioguide"]
        names = [x for x in (n.get("official_full"),
                             (n.get("first", "") + " " + n.get("last", "")).strip(),
                             (n.get("nickname", "") + " " + n.get("last", "")).strip()) if x]
        if b in roster_bio or any(_norm(x) in roster_names for x in names):
            continue
        t = m["terms"][-1]
        seat = t["state"] + (f"-{t['district']}" if t.get("district") else "")
        missing.append((n.get("official_full") or names[0],
                        "Senate" if t["type"] == "sen" else "House", seat))
...
def main():
    ...
    # Roster drift against the official source, both directions.
    try:
        _sen = json.load(open(os.path.abspath(SENATE_PATH)))
        _hou = json.load(open(os.path.abspath(HOUSE_PATH)))
        _bio = json.load(open(os.path.join(os.path.dirname(FEC_PATH), "bioguide.json")))
    except Exception:
        _sen = _hou = _bio = None
    if _sen is not None:
        departed, missing_new = roster_drift(_sen, _hou, _bio or {})
        if departed is not None:
            for nm, ch, seat in departed:
                err("in the roster but no longer serving", nm,
                    f"{ch} {seat}: not in congress-legislators. Died, resigned or "
                    f"left office; still counted in every chamber median and rank.")
            for nm, ch, seat in missing_new:
                err("serving but missing from the roster", nm,
                    f"{ch} {seat}: in congress-legislators but not in senate.json "
                    f"or house.json. Their seat shows the wrong member or none.")
