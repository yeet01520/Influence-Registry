...
# is 120/minute (7,200/hour), and on that tier the hourly cap does not apply.
#
# Evidence for the value below: a dry run on this key sustained 110 calls/min
# for 8 straight minutes (about 660 calls) with exactly ONE 429. A personal
# 1,000/hour key would have hit its wall near minute 9 and refused everything
# after. That is the upgraded per-minute tier, so MINUTE_BUDGET is the real
# guard and HOURLY_BUDGET is set well above any run this script will make.
#
# If this key turns out to be the personal tier after all, the failure is safe
# and loud: calls start 429ing near 1,000, MAX_CONSECUTIVE_429 trips, partial
# progress is saved, and the run exits 2 telling you to resume. In that case
# set HOURLY_BUDGET back to 950.
HOURLY_BUDGET = 7000         # per-minute throttle is the real guard on this key
...
def _env_true(name):
    """True if the named env var is set to 'true' (case/space insensitive)."""
...
# Flags come from EITHER the command line OR the workflow inputs passed
# through as environment variables. Reading the environment directly means a
# broken shell arg-building step can no longer silently turn a commit run
# into a dry run, which is exactly what happened on runs 1 through 4.
COMMIT = ("--commit" in sys.argv) or _env_true("COMMIT_INPUT")
RESUME = ("--resume" in sys.argv) or _env_true("RESUME_INPUT")
LIMIT = arg_value("--limit", None, int) or _env_int("LIMIT_INPUT")
SAVE_EVERY = arg_value("--save-every", None, int) or _env_int("SAVE_EVERY_INPUT") or 25
...
def split_for_candidate(cid):
    """
    Sum PAC / individual / party contributions across the cycles the FEC
    totals endpoint returns for this candidate id.

    DOUBLE-COUNTING GUARD (verified against live FEC data, not assumed)
    -------------------------------------------------------------------
    /candidate/{id}/totals/ returns EACH CYCLE TWICE. One row carries the
    cycle number; a duplicate row carries cycle=None with identical values.
    Checked on AOC (H8NY15148): 10 rows for 5 cycles. Summing every row
    gives $163,469,588 individual. Keeping only the rows that carry a cycle
    gives $81,734,794, which matches her known receipts.

    Note what does NOT fix this: passing full_election=false. The response
    is unchanged, and no row carries a full_election value at all, so
    filtering on that field is a no-op. The cycle field is the real
    discriminator.

    Returns (individual, pac, party, self_funded, transfers_in) or None on
    failure. self_funded = candidate_contribution + loans_made_by_candidate.
    """
    data = get(f"/candidate/{cid}/totals/", {"per_page": 100})
    if data is None or "results" not in data:
        return None
    ind = pac = party = 0.0
    self_gift = self_loan = transfers = 0.0
    seen_cycles = set()
    for r in data["results"]:
        cyc = r.get("cycle")
        if cyc is None:          # the duplicate half of the response
            continue
        if cyc in seen_cycles:   # belt and braces
            continue
        seen_cycles.add(cyc)
        ...
        # Money the campaign took in that nobody DONATED. Same endpoint, same
        # call; these fields were already coming back and being discarded.
        #
        # Without them a self-funder reads as a small-donor candidate: Rick
        # Scott shows 92% individual once the denominator is contributions,
        # which is true of his contributions and hides that $89.8M of his
        # campaign was his own money ($65.7M gifted plus $24.1M in loans).
        self_gift += float(r.get("candidate_contribution") or 0)
        self_loan += float(r.get("loans_made_by_candidate") or 0)
        transfers += float(r.get("transfers_from_other_authorized_committee") or 0)
    return (ind, pac, party, self_gift + self_loan, transfers)
...
def main():
    ...
    print("                    env : COMMIT_INPUT=%r RESUME_INPUT=%r LIMIT_INPUT=%r"
          % (os.environ.get("COMMIT_INPUT"), os.environ.get("RESUME_INPUT"),
             os.environ.get("LIMIT_INPUT")))
    ...
    api_failed = []
    examples = []
    overcount = []   # members whose contributions exceed their total receipts
    backup_done = False
    stopped_early = None
    ...
    for idx, (name, rec) in enumerate(members):
        ...
        rec["pac_pct"] = pac_pct
        # Not scoring inputs. Self-funding is not influence from anyone else,
        # so it stays out of the PAC-dependence score; it is shown on the
        # profile so a self-funded campaign is not mistaken for a
        # small-donor one.
        rec["self_funded"] = round(tot_self)
        rec["transfers_in"] = round(tot_transfers)
        updated += 1

        if pac_pct is not None and pac_pct <= 1 and denom > 1_000_000:
            examples.append((name, round(tot_pac), round(denom), pac_pct))

        # Sanity guard. Contributions are a SUBSET of total receipts and can
        # never exceed them. If they do, we are double counting rows again.
        # This is the check that would have caught the cycle=None duplication
        # on the first run instead of the fourth.
        tr = rec.get("total_raised")
        if isinstance(tr, (int, float)) and tr > 0 and denom > tr * 1.05:
            overcount.append((name, round(denom), round(tr), denom / tr))
    ...
    if overcount:
        overcount.sort(key=lambda x: -x[3])
        print()
        print("!! SANITY FAILURE: contributions exceed total receipts for "
              f"{len(overcount)} members. This means rows are being counted")
        print("!! more than once. DO NOT trust the dollar figures from this run.")
        for n, d, tr, mult in overcount[:10]:
            print(f"    {n:26s} contributions ${d:>13,}  vs receipts ${tr:>13,}  ({mult:.2f}x)")
    else:
        print()
        print("Sanity check PASSED: no member's contributions exceed their total receipts.")
