...
5. CATEGORIES:
     - aipac        (from trackaipac_page.txt — PACs + IE; aipac_lobby_donors preserved separately)
...
FIRST_NAME_ALIASES = {
    ...
    "val":      {"valerie"},
    "trish":    {"patricia"},
    # Added to recover OpenSecrets sector matches for members whose CSV row uses
    # a formal first name the roster shortens (verified against the 1990-2024 CSVs).
    "vern":     {"vernon"},
    "don":      {"donald"},
    "jack":     {"john"},
    "johnny":   {"john", "jonathan"},
    "nellie":   {"nelida", "eleanor"},
    "chuy":     {"jesus"},
}
...
# State abbreviation → full name. The member roster (senate.json/house.json)
# stores two-letter abbreviations ("CT"), while the OpenSecrets sector CSVs
# store full state names ("Connecticut"). Without canonicalizing both sides,
# the hard state filter in _best_csv_match rejects every CSV row for a member
# (abbrev != full name), zeroing out their entire sector breakdown. This map
# lets us compare them on equal footing.
...
def _canon_state(state):
    """Canonicalize a state to its full lowercase name so that an abbreviation
    ('CT') and a full name ('Connecticut') compare equal. Returns '' for empty."""
    if not state:
        return ""
    s = _normalize_oil(state)
    return _STATE_ABBREV_TO_NAME.get(s, s)

def _normalize_oil(name):
    """Normalize for token-set matching: lowercase, strip accents (á→a, ñ→n),
    hyphens→spaces, strip noise. Accent stripping is required because the roster
    carries accented surnames (Sánchez, Giménez) while the OpenSecrets CSVs use
    plain ASCII (Sanchez, Gimenez); deleting accented chars instead of
    transliterating them would turn 'sánchez' into 'snchez' and break matching."""
    import re as _re
    import unicodedata as _ud
    name = name.lower()
    # Decompose accented characters and drop the combining marks, so 'á' → 'a'.
    name = _ud.normalize('NFKD', name)
    name = ''.join(c for c in name if not _ud.combining(c))
...
def _best_csv_match(member_name, member_state, csv_data):
    ...
    norm_state  = _canon_state(member_state) if member_state else ""
    ...
    for (csv_key, csv_state), amt in csv_data.items():
        ...
        # CSV row carries a state, require a match after canonicalizing both
        # to full state names (member roster uses abbreviations like "CT",
        # CSVs use full names like "Connecticut"). This is the critical fix
        # for same-name-different-state collisions.
        if norm_state and csv_state and _canon_state(csv_state) != norm_state:
            continue
...
# Flattened (source_committee_id, sector) pairs for targeted Schedule A
# lookups in fetch_all_receipts Step 3. Derived from SECTOR_CMTE_IDS.
_TRACKED_SOURCE_PACS = [
    (cid, sector)
    for sector, cids in SECTOR_CMTE_IDS.items()
    for cid in cids
]
    ...
    for office in offices_to_search:
        ...
        for sp in search_tries:
            ...
            for r in data["results"]:
                ...
                # HARD REJECT: never attach a Presidential ID (P-prefix) to a
                # congressional member. Presidential candidates have no state
                # in their FEC record, so the state check below won't catch them.
                if cid.startswith("P"):
                    continue

                # HARD STATE CHECK (API record) — reject any candidate whose
                # state field does not match the member's declared state.
                if state and r.get("state") and r.get("state") != state:
                    continue

                # HARD STATE CHECK (candidate ID prefix) — FEC candidate IDs
                # encode the state directly: H{yr}{ST}{dist} and S{yr}{ST}{seq}.
                # Some API records have a null/empty state field even though the
                # ID encodes a different state. This catches S4OR00123 leaking
                # into an NJ member when the API returns the record without a
                # state field populated.
                if len(cid) >= 4 and cid[0] in ("H", "S"):
                    id_state = cid[2:4]
                    if state and id_state and id_state != state:
                        continue
...
def get_all_committees(candidate_ids_with_office):
    ...
    # Last-resort fallback: accept ANY committee type.
    # This handles territorial Delegates and the Resident Commissioner whose
    # campaign committees may be filed under non-H/S/P type codes (e.g. "D"
    # for Delegate). Without this, members like Amata Coleman Radewagen
    # (American Samoa) come back as "no committees found" even when their
    # candidate ID is correctly resolved.
    if not all_cmte_ids:
        for (cid, office) in candidate_ids_with_office:
            data = get(f"/candidate/{cid}/committees/", {"per_page": 50})
            if data and data.get("results"):
                for r in data["results"]:
                    ...
                    cmte_type = r.get("committee_type", "")
                    designation = r.get("designation", "")
                    # Still filter by designation (P=Principal, A=Authorized)
                    # so we exclude PACs and party committees that happen to
                    # be linked to the candidate. Just drop the type filter.
                    if (cmte_id
                            and cmte_id not in seen
                            and designation in ("P", "A")):
                        all_cmte_ids.append(cmte_id)
                        seen.add(cmte_id)
                        print(f"  [committee type='{cmte_type}' accepted via last-resort fallback]")
...
def get_active_cycles_for_member(office):
    ...
    """
    # Current FEC two-year cycle, computed dynamically so the list never
    # goes stale: cycles are even years, covering (year-1, year).
    # e.g. in 2025 or 2026 the current cycle is 2026.
    import datetime
    _y = datetime.date.today().year
    current_cycle = _y if _y % 2 == 0 else _y + 1
    if office == "S":
        return list(range(current_cycle, 1988, -2))   # current → 1990
    else:
        return list(range(current_cycle, 1998, -2))   # current → 2000
...
def fetch_all_receipts(committee_ids, active_cycles, member_name):
    """
    LEAN ARCHITECTURE (replaces full Schedule A pagination, which required
    100K+ API calls for senior members and made full runs infeasible under
    FEC rate limits):

      total_raised : /committee/{id}/totals/        (official, all receipts,
                     includes unitemized money that never appears in Sch A)
      grassroots   : individual_unitemized_contributions from the same
                     totals call  PLUS  the <=$200 itemized bucket from
                     /schedules/schedule_a/by_size/ (size=0). The FEC
                     computes both with proper conduit/memo dedup.
      sectors/nra  : Schedule A filtered to is_individual=false — committee
                     (PAC) receipts only, a few hundred rows per member —
                     matched by SECTOR_CMTE_IDS then SECTOR_KEYWORDS.

    Headline sectors (fossil_fuels, pharma, defense, finance, tech) are NOT
    derived here — they come from OpenSecrets CSVs and overwrite these keys
    after this function returns. aipac comes from TrackAIPAC (Step 5).

    Returns (sector_totals, grassroots, total_raised) — same contract as
    the old implementation, so downstream merge code is unchanged.
    """
    all_sectors = list(SECTOR_KEYWORDS.keys()) + [
        "aipac", "fossil_fuels", "pharma", "defense", "finance", "tech"]
    sector_totals = {k: 0 for k in all_sectors}
    grassroots = 0.0
    total_raised = 0.0
    active = set(active_cycles)

    for cmte in committee_ids:
        # ── 1. Official per-cycle totals: total_raised ──────────────────
        # /committee/{id}/totals/ returns one row per cycle the committee
        # filed. We keep ONLY rows whose cycle is in this member's active
        # set, and dedup by cycle so a committee that appears under multiple
        # members (shared/joint/inherited) can't double count. This fixes
        # the inflated-total bug (e.g. a member showing $243M because an
        # unrelated committee's cycles were summed in).
        # total_raised : sum of per-cycle `receipts` across the member's
        #                active cycles (career total).
        # grassroots   : sum of per-cycle `individual_contributions`
        #                — ALL individual donations regardless of size,
        #                as reported on FEC Form 3/3X Line 11a. This
        #                includes both itemized (>$200) and unitemized
        #                (≤$200) individual contributions. Previously
        #                used individual_unitemized_contributions which
        #                only captured the sub-$200 lump-sum bucket.
        #
        # IMPORTANT: grassroots comes from THIS totals call, not from
        # /schedules/schedule_a/by_size/. The by_size size=0 bucket draws
        # from ITEMIZED Schedule A records and is heavily inflated (e.g.
        # Cruz showed $45.7M in "<=$200" money for the 2024 off-cycle year,
        # which is impossible). individual_unitemized_contributions is the
        # correct, deduped source and costs zero extra API calls.
        seen_cycles = set()
        t = get(f"/committee/{cmte}/totals/", {"per_page": 100})
        if t and t.get("results"):
            for row in t["results"]:
                cyc = row.get("cycle")
                if not cyc or cyc not in active or cyc in seen_cycles:
                    continue
                seen_cycles.add(cyc)
                total_raised += row.get("receipts") or 0
                grassroots   += (row.get("individual_itemized_contributions") or 0) + (row.get("individual_unitemized_contributions") or 0)
        time.sleep(0.1)

        # ── (sector/NRA per-receipt computation removed) ────────────────
        # No per-receipt sector computation here. The five headline sectors
        # (pharma/defense/finance/tech/fossil) come from OpenSecrets CSVs and
        # are written AFTER this function returns. NRA — the only sector that
        # was ever computed from FEC receipts — is not currently displayed
        # anywhere on the site, so we do not spend API calls on it. If NRA
        # display is wanted later, compute it in a dedicated, verified pass
        # using a confirmed FEC filter (committee_id = NRA PAC, recipient =
        # member committee) rather than scanning a member's full receipts.
        pass

    return sector_totals, round(grassroots), round(total_raised)
...
def fetch_official_totals(candidate_ids_with_office):
    """
    Get official FEC total receipts, individual contributions, and most
    recent active cycle.

    Uses /candidates/{id}/totals/ WITHOUT a cycle filter to get all
    cycles at once. individual_contributions is the FEC Line 11a total
    covering ALL individual donations (itemized + unitemized) and exists
    on the candidate totals endpoint for all office types.
    """
    total       = 0
    indiv_total = 0
    max_cycle   = 0
    ...
    for (cid, _) in candidate_ids_with_office:
        ...
        if data and data.get("results"):
            for r in data["results"]:
                ...
                if receipts > 0:
                    total       += receipts
                    indiv_total += r.get("individual_contributions") or 0
                    if cycle > max_cycle:
                        max_cycle = cycle
        time.sleep(0.2)

    return round(total), round(indiv_total), max_cycle if max_cycle else 2024
...
def main():
    ...
    for i, member in enumerate(all_members):
        ...
        try:
            ...
            sectors, _grassroots_unitemized, receipts_total = fetch_all_receipts(
                committee_ids, active_cycles, name
            )
            time.sleep(0.2)

            # ── STEP 4b: Individual contributions from candidate totals ──────
            # fetch_official_totals uses /candidates/{cid}/totals/ which has
            # individual_contributions = ALL individual donors (itemized +
            # unitemized). This replaces the committee-level lookup which
            # only had individual_unitemized_contributions for F3 filers.
            _, grassroots, _ = fetch_official_totals(candidate_ids)
            time.sleep(0.2)


            # ── STEP 5: AIPAC — from TrackAIPAC data (most complete) ────────
            # TrackAIPAC captures three components: direct PAC contributions
            # (aipac_pacs), independent expenditures (aipac_ie), and bundled
            # Lobby Donors (aipac_lobby_donors).
            #
            # METHODOLOGY: the headline `aipac` field equals pacs + ie only.
            # aipac_lobby_donors is preserved in the record for reference
            # but is intentionally excluded from the displayed total — it
            # represents a different funding mechanism (bundled individual
            # contributions to a candidate's own committee) than direct PAC
            # money and IE, and conflating the three overstates AIPAC's
            # direct influence. This matches TrackAIPAC's per-component
            # reporting and what the validator's H2 invariant requires.
            ta = get_aipac_from_trackaipac(name)
            sectors["aipac"] = ta["pacs"] + ta["ie"]
