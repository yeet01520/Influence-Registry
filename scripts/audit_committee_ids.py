...
def main():
    ...
    if blocked:
        print(f"\nNEEDS REVIEW ({len(blocked)}): every committee on file belongs to "
              "someone else, so nothing was removed.")
        for m, bad in blocked:
            print(f"  {m:24s} only committees are {bad}")
    ...
    with open(os.path.abspath(REPORT_PATH), "w") as fh:
        json.dump({"confirmed": confirmed, "unlinked": unlinked,
                   "needs_review": [m for m, _ in blocked],
