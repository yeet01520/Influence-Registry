...
# State names (multi-word first) used to strip the "Download Graphics <State>"
# UI boilerplate that TrackAIPAC glues in front of each member's name. Without
# this, names like "Mark Kelly" get stored as "download graphics arizona mark
# kelly", fail to match, and get wrongly zeroed.
_STATES = ("District of Columbia|Northern Mariana Islands|American Samoa|"
    "Puerto Rico|Virgin Islands|New Hampshire|New Jersey|New Mexico|New York|"
    "North Carolina|North Dakota|Rhode Island|South Carolina|South Dakota|"
    "West Virginia|Alabama|Alaska|Arizona|Arkansas|California|Colorado|"
...
# First-name nickname/formal equivalences, so "James Risch" still matches a page
# entry stored as "jim risch" while "Lindsey Graham" does NOT match "darline
# graham" (different people who merely share a surname).
_NICK = [
    {'james','jim','jimmy','jamie'},{'joseph','joe'},{'michael','mike'},
    {'robert','rob','bob','bobby'},{'william','will','bill','billy'},
    {'richard','rick','rich','dick'},{'thomas','tom','tommy'},{'jefferson','jeff'},
    {'jerrold','jerry','gerald'},{'john','johnny','jack'},{'abraham','abe'},
    {'daniel','dan','danny'},{'christopher','chris'},{'anthony','tony'},
    {'edward','ed','eddie'},{'ronald','ron'},{'kenneth','ken'},{'gregory','greg'},
    {'nicholas','nick'},{'benjamin','ben'},{'matthew','matt'},{'andrew','andy','drew'},
    {'samuel','sam'},{'charles','charlie','chuck'},{'patrick','pat'},{'timothy','tim'},
    {'david','dave'},{'stephen','steven','steve'},{'lawrence','larry'},{'raymond','ray'},
    {'frederick','fred'},{'theodore','ted','teddy'},{'margaret','maggie','peggy'},
    {'elizabeth','liz','beth','betty'},{'katherine','kathy','kate','katie'},
    {'deborah','debbie','deb'},{'jennifer','jen','jenny'},{'patricia','patty'},
]

def _same_first(a, b):
    """True if two first-name tokens plausibly denote the same person."""
    if a == b:
        return True
    if a.startswith(b) or b.startswith(a):   # maria / mariaelvira, marianette / mariannette
        return True
    for s in _NICK:
...
def find_match(name, ta_data):
    ...
    # A surname-only match is NOT enough: the first name must also be compatible,
    # otherwise different people sharing a surname get cross-matched (Lindsey vs
    # Darline Graham, Sherrod vs Shontel Brown). Page keys may carry leftover
    # prefix tokens, so we accept if the member's first name matches ANY token.
    if len(hits) == 1:
        only_key = next(iter(hits))
        if any(_same_first(first, t) for t in only_key.split()):
            return hits[only_key]
        return None
    exact = {k: v for k, v in hits.items() if any(_same_first(first, t) for t in k.split())}
    if len(exact) == 1:
...
def parse_live(text):
    ...
    for i, mk in enumerate(markers):
        ...
        # Remove status annotations that get glued in front of the member name
        # (e.g. "...AIPAC, AMPUp for Re-Election Susan Collins" where the name is
        # "Susan Collins"). These phrases appear right before the real name, so
        # we strip any occurrence and keep what follows as the name candidate.
        pre = re.sub(
            r'(?:Download Graphics(?:\s+(?:' + _STATES + r'))?|'
            r'Up for Re-?[Ee]lection|Next [Ee]lection:?\s*\d{0,4}|'
            r'Retiring\s*\d{0,4}|Running for [A-Za-z. ]+?\d{2,4}|'
            r'Defeated in [A-Za-z ]+?\d{2,4}|Special [Ee]lection:?\s*[A-Za-z0-9, ]*?\d{2,4}|'
            r'Ran for [A-Za-z. ]+?\d{2,4}|Signed the PEACE Pledge|'
            r'Track AIPAC Approved!?)',
            ' ', pre)
        # Also cut full explanatory sentences that can precede a name
        pre = re.sub(r'(?:This |We encourage )[^.]*\.', ' ', pre)
        pre = pre.rstrip()
        nm = re.search(
...
def main():
    ...
    changes = []
    kept_unmatched = []   # unmatched but had nonzero data -> value KEPT, needs review
    matched = zeroed = 0
    for name, d in fec.items():
        ...
        if m:
            ...
            # SAFETY: never wrongly zero a member who currently has AIPAC data
            # just because the scraper failed to match their name this run.
            # Keep their existing values untouched and flag for manual review.
            # Members who genuinely left office (e.g. resigned) are the only ones
            # that should reach $0, and those are handled by manual review, not
            # by a silent auto-zero here.
            if old_total != 0:
                kept_unmatched.append((name, old_total))
                # leave d["aipac"], aipac_pacs, aipac_ie, sources as-is
            else:
    ...
    print(f"  Unmatched->$0  : {zeroed - len(kept_unmatched)} members (had no prior AIPAC data)")
    print(f"  Unmatched KEPT : {len(kept_unmatched)} members (had data; VALUE KEPT, review below)")
    print(f"  CHANGES        : {len(changes)} members whose AIPAC total or IE changed")
    print(f"{'='*60}")
    if kept_unmatched:
        print("\n  REVIEW — unmatched members whose existing value was KEPT (not zeroed):")
        print("  (If any of these actually LEFT office, zero them manually. If the")
        print("   scraper just missed the name, the kept value may be stale.)")
        for name, ot in sorted(kept_unmatched, key=lambda x: -x[1]):
            print(f"    {name:<30}{ot:>12,}  (kept)")
    if changes:
