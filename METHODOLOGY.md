...
- **Grassroots-tagged members** retain a static editorial baseline (default
  12). Because the v3 formula already scores individual-funded members low,
  this baseline and the formula now point in the same direction.
- **Challengers** are handled by funding history. A brand-new candidate with no
  prior federal donation record receives a manual editorial baseline set in
  `corruption_score`. A challenger who is already a sitting official with a real
  donation record (for example, a state officeholder running for federal
  office) has no baseline set and is scored by the standard v3 formula on their
