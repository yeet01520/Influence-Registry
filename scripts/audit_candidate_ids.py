...
def names_match(a, b):
    """Loose match on names — handle 'Richard Durbin' vs 'Dick Durbin' etc."""
    na = normalize_name(a)
    nb = normalize_name(b)
    if na == nb:
        return True
    # First+last match (handles middle initials)
    a_parts = na.split()
    b_parts = nb.split()
    if len(a_parts) >= 2 and len(b_parts) >= 2:
        if a_parts[0] == b_parts[0] and a_parts[-1] == b_parts[-1]:
            return True
    # Last name + same start of first name (e.g. Dick/Richard, Bob/Robert)
    NICKNAME_PAIRS = [
    ...
    if len(a_parts) >= 2 and len(b_parts) >= 2 and a_parts[-1] == b_parts[-1]:
        for nick, full in NICKNAME_PAIRS:
            if (a_parts[0] == nick and b_parts[0] == full) or (a_parts[0] == full and b_parts[0] == nick):
                return True
    return False
