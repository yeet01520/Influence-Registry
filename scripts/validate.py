...
HARD CHECKS (deploy-blocking errors)
  ...
  [H2] AIPAC internal math:    fec.aipac == aipac_pacs + aipac_ie
...
def load_html(path):
    ...
    # scores.json is the authoritative score source (written by compute_scores.py,
    # which runs before this validator in the workflow). profiles.json's
    # corruption_score field is legacy/stale and no longer displayed, so checks
    # that judge scores must read scores.json, not profiles.json.
    try:
...
def load_trackaipac(path="trackaipac_page.txt"):
    """Parse TrackAIPAC's published per-member figures. Returns the full
    'Israel Lobby Total' (PACs + IE + Lobby Donors combined), which is what
    AIPAC_DATA holds. Earlier versions extracted only the PACs portion,
    which made S2 comparisons report apples-to-oranges drift."""
    data = {}
    p = Path(path)
    if not p.exists():
    ...
    for line in p.read_text(encoding="utf-8").split("\n"):
        ...
        if "Israel Lobby Total" not in line and "Lobby Total" not in line:
            continue
        m = re.match(r"([A-Z][A-Z\s\'\-\.]+?)\s+[A-Z]{2}-(?:[A-Z\d]+|At Large)\s*[\[(][RDI][\])].*?Israel Lobby Total:\s*\$([\d,]+)", line)
        if m:
            name = m.group(1).strip().title()
            total = int(m.group(2).replace(",",""))
            data[normalize(name)] = total
    return data
...
DONATIONS_TO_FEC = {
    ...
    "defense":     "defense",
    "tech":        "tech",
    "aipac":       "aipac",  # card aipac string equals fec.aipac (pacs + ie)
}
...
def check_aipac_internal_math(d):
    """[H2] fec.aipac must equal aipac_pacs + aipac_ie.

    This reflects the TrackAIPAC-consistent methodology: the headline AIPAC
    figure represents direct PAC contributions plus independent expenditures
    in support of the candidate. aipac_lobby_donors is preserved in the data
    for reference but is intentionally excluded from the displayed total.
    Any disagreement indicates corruption or manual edit drift. Run
    scripts/fix_fec_consistency.py to repair."""
    print("\n[H2] AIPAC internal math (aipac == pacs + ie)...")
    issues = 0
    for name, fec in d["fec_v8"].items():
        pacs   = fec.get("aipac_pacs", 0) or 0
        ie     = fec.get("aipac_ie", 0) or 0
        stored = fec.get("aipac", 0) or 0
        if abs((pacs + ie) - stored) > 100:
            err(f"AIPAC math broken — {name}: stored aipac=${stored:,} but pacs+ie=${pacs+ie:,}")
            issues += 1
    if issues == 0:
...
def check_sit_internal_math(d):
    ...
    Components: aipac_pacs + aipac_ie + fossil + pharma + defense + finance
    + tech + nra. We sum aipac_pacs + aipac_ie (which together equal the
    aipac field under the current methodology) rather than aipac itself to
    keep the formula explicit about what's being counted. aipac_lobby_donors
    is preserved in the data but excluded from SIT, matching the change to
    the headline aipac figure."""
...
def check_score_sanity(d):
    """[H5] Score sanity: a non-grassroots member with real tracked PAC money
    should not score as effectively zero — that signals a scoring break (e.g. the
    formula never saw their sector money). We deliberately do NOT assert an
    arbitrary score floor: under the current log formula
    (33*log10(1+total/100_000)), $0.5M-3M of tracked money legitimately yields
    scores in the 30s-40s, so a hard '<50' cutoff produces false positives.
    We also skip members who pledged $0 corporate PAC (their low score is correct
    by design) and appointed members scored N/A."""
    ...
    for name, prof in d["profiles"].items():
        ...
        if not isinstance(prof, dict):
            continue
        # Authoritative score lives in scores.json (pct). Fall back to the legacy
        # profiles.json corruption_score only when scores.json is unavailable
        # (e.g. Phase 1 inline validation), since that field is stale.
        score_rec = scores.get(name) or {}
        if "pct" in score_rec:
            score = score_rec.get("pct", 0) or 0
            # Appointed members with no campaign data are intentionally N/A — skip.
            if score_rec.get("no_campaign"):
                continue
        else:
            score = prof.get("corruption_score", 0) or 0

        # Respect the $0-corporate-PAC pledge: scoring intentionally excludes a
        # pledged member's sector money, so a low score is correct, not an anomaly.
        ct = str((prof.get("donations") or {}).get("corporate_total") or "").lower()
        if "$0" in ct:
            continue
        ...
        # Flag only a genuine break: substantial confirmed PAC money but a
        # near-zero score (the formula would give >=20 for even $100K).
        if confirmed_pac > 500_000 and score < 10:
            err(f"Score anomaly — {name}: score={score} but confirmed PAC money=${confirmed_pac:,} "
                f"(near-zero score with real PAC money signals a scoring break)")
            issues += 1
    if issues == 0:
...
def check_corporate_total(d):
    ...
    for name, prof in d["profiles"].items():
        ...
        # Mirror the runtime corporate_total resolution done by both compute_scores.py
        # and index.html's applyFECData: when a member has FEC special_interest_total
        # and is not a "$0 corporate" pledge, the displayed corporate_total is
        # overwritten with "$<special_interest_total>". The stored profiles.json value
        # ("Takes corporate PAC money", stale "$X career", etc.) is never shown. So we
        # validate the EFFECTIVE value that actually ships, not the stale stored one.
        fec = d["fec_v8"].get(name, {})
        sit = fec.get("special_interest_total")
        if sit and "$0 corporate" not in str(ct).lower():
            ct = "$" + format(int(sit), ",")
        ...
        # Appointed-member placeholder (no federal campaign finance history)
        if "no federal campaign" in ct_lower:
            continue
        ...
        # A resolved "$<number>" (from special_interest_total) is a valid effective value.
...
def check_aipac_vs_fec_pacs(d):
    """[S1] AIPAC_DATA holds the full Israel Lobby Total (PACs + IE + Lobby
    Donors, per TrackAIPAC), while fec.aipac under the current methodology is
    PACs + IE only. So AIPAC_DATA is EXPECTED to exceed fec.aipac by roughly
    aipac_lobby_donors. Rather than flag every member where the two differ
    (which is ~all of them and pure noise), we verify the expected
    relationship holds: AIPAC_DATA ≈ fec.aipac + aipac_lobby_donors. A warning
    here means the relationship is broken — e.g. AIPAC_DATA has money the FEC
    record knows nothing about (fec.aipac and lobby both 0), or the components
    don't add up — which is genuinely worth a look."""
    print("\n[S1] AIPAC_DATA vs fec.aipac (relationship check)...")
    mismatches = 0
    for name, aipac_val in d["aipac"].items():
        rec = d["fec_v8"].get(name, {})
        fec_aipac = rec.get("aipac", 0) or 0
        lobby     = rec.get("aipac_lobby_donors", 0) or 0
        expected  = fec_aipac + lobby
        # Tolerance: 2% of the expected total, floor $5,000, to absorb rounding
        # and minor cycle-timing differences between the two sources.
        tol = max(5000, expected * 0.02)
        if abs(aipac_val - expected) > tol:
            warn(f"AIPAC relationship broken — {name}: AIPAC_DATA=${aipac_val:,} "
                 f"≠ fec.aipac (${fec_aipac:,}) + lobby (${lobby:,}) = ${expected:,}")
            mismatches += 1
    if mismatches == 0:
        print(f"  ✅ All {len(d['aipac'])} AIPAC entries match the expected PACs+IE+lobby relationship")
    else:
        print(f"  ⚠️  {mismatches} AIPAC relationship anomalies (warning)")
...
def _load_fetch_matcher(csv_dir):
    """Import fetch_fec_data.py (co-located in scripts/) and load its sector
    CSVs, returning a dict of {sector: getter(name, state)}. Reusing the fetch
    script's matcher guarantees S3 compares fec.json against the EXACT same
    name-matching logic that produced it (state canonicalization + accent
    transliteration + nickname aliases), so the two can never disagree over a
    name-form difference. Returns None if the script or CSVs are unavailable."""
    import importlib.util, os
    here = Path(__file__).resolve().parent
    script = here / "fetch_fec_data.py"
    ...
    loaders = [
        ...
        ("get_tech_amount",    "load_tech_data",
    ...
    getters = {}
    sector_for = {"get_oil_amount": "oil", "get_pharma_amount": "pharma",
                  "get_defense_amount": "defense", "get_finance_amount": "finance",
                  "get_tech_amount": "tech"}
    any_loaded = False
    for getter_name, loader_name, (rep_csv, sen_csv) in loaders:
        loader = getattr(mod, loader_name, None)
        getter = getattr(mod, getter_name, None)
        if loader is None or getter is None:
            continue
        rep_p, sen_p = Path(csv_dir) / rep_csv, Path(csv_dir) / sen_csv
        if not rep_p.exists() and not sen_p.exists():
            continue
        loader(str(rep_p), str(sen_p))
        getters[sector_for[getter_name]] = getter
        any_loaded = True
    return getters if any_loaded else None

def check_sectors_vs_csv(d, csvs, members):
    """[S3] FEC sector totals vs OpenSecrets CSV totals, compared using the
    SAME matcher fetch_fec_data.py uses to populate fec.json. Because both
    sides now run identical name-matching, a warning here means a genuine
    value mismatch (e.g. fec.json went stale, or a CSV was updated), not a
    spurious name-form difference. Falls back to the legacy normalize-only
    lookup if the fetch matcher can't be imported."""
    print("\n[S3] Sector values vs CSV source files...")
    members_by_name = {m["name"]: m for m in members}

    getters = _load_fetch_matcher(d.get("_data_dir", Path(".")).parent / "raw"
                                  if d.get("_data_dir") else "data/raw")
    # Also try the conventional csv dir passed via load_csvs base.
    if getters is None:
        getters = _load_fetch_matcher("data/raw")

    if getters is None and not any(csvs.values()):
    ...
    for name, fec in d["fec_v8"].items():
        ...
        if name not in members_by_name:
            continue
        state = members_by_name[name].get("state", "")
        n = normalize(name)
        n = CSV_LOOKUP_FIXES.get(n, n)
        for csv_sector, fec_key in SECTOR_MAP.items():
            # Prefer the shared fetch matcher (state-aware, alias-aware).
            if getters and csv_sector in getters:
                csv_val = int(getters[csv_sector](name, state) or 0)
            else:
