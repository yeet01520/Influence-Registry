...
def candidate_cycles(cand_id, api_key, fallback):
    ...
    if not years:
        return fallback
    # Election years are the cycles themselves. Include everything from the
    # first through the latest cycle so a gap year with spending is not missed.
    return list(range(min(years), LATEST_CYCLE + 1, 2))
