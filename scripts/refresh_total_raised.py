...
def fetch_committee_total(committee_id):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            indiv = sum(
                (cycle.get("individual_itemized_contributions") or 0) +
                (cycle.get("individual_unitemized_contributions") or 0)
    ...
    for i, (name, entry) in enumerate(sorted(fec.items())):
        ...
        for cid in committees:
            ...
            if not may_count(cid, name):
                continue
            result = fetch_committee_total(cid)
            if result is None or result[0] is None:
                any_failed = True
                continue
            c_total, c_indiv = result
            new_total += c_total
            new_indiv += c_indiv
            time.sleep(SLEEP_BETWEEN_CALLS)
