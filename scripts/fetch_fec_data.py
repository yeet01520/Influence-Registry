...
def fetch_all_receipts(committee_ids, active_cycles, member_name):
    ...
    at startup (trackaipac_page.txt). The headline aipac total is
    aipac_pacs + aipac_ie; aipac_lobby_donors is preserved separately but
    not folded into the displayed total. The aipac key is initialized to 0
    and filled in Step 5 of the main loop via get_aipac_from_trackaipac().
    ...
    for cmte_id in committee_ids:
        for cycle in active_cycles:
            ...
            while True:
                ...
                if last_index:
                    # FEC keyset pagination requires passing back EVERY key in
                    # pagination.last_indexes (e.g. last_index AND
                    # last_contribution_receipt_date when sorting by date).
                    # Passing only last_index made page 2 return empty,
                    # silently capping every committee-cycle at 100 receipts.
                    params.update(last_index)
                ...
                for item in results:
                    ...
                    # ── CONDUIT LUMP SKIP (prevents double counting) ─────
                    # ActBlue/WinRed-style conduits report a non-memo lump
                    # PLUS memo-coded (memo_code=X) per-donor earmark details
                    # for the SAME dollars. Counting both inflates totals
                    # (symptom: grassroots exceeding total raised). We skip
                    # the lump and keep the per-donor memo details, which
                    # carry the employer info needed for sector matching.
                    _contrib_l = (item.get("contributor_name") or "").lower()
                    _is_memo = (item.get("memo_code") or "").upper() == "X"
                    _is_conduit = any(x in _contrib_l for x in (
                        "actblue", "winred", "democracy engine", "anedot"))
                    if _is_conduit and not _is_memo:
                        continue
                    ...
                    # Conduit lumps are skipped above. Per-donor earmark
                    # details flow through the standard rules (≤$200 IND →
                    # grassroots; employer keywords → sectors), so no
                    # aggregator special-case is needed here anymore.
                ...
                if not last_indexes.get("last_index"):
                    break
                last_index = dict(last_indexes)  # carry ALL keyset values forward
                time.sleep(0.1)
...
def main():
    ...
    for i, member in enumerate(all_members):
        ...
        try:
            ...
            # ── STEP 4b: Official total_raised from the totals endpoint ──────
            # /committee/{id}/totals/ returns the committee's official
            # per-cycle receipts (including unitemized small-dollar money that
            # never appears in Schedule A). One cheap call per committee,
            # paginated per-cycle rows. Far more accurate than summing
            # Schedule A receipts. Falls back to the Schedule A sum if the
            # endpoint returns nothing.
            totals_sum = 0
            for _cid in committee_ids:
                _t = get(f"/committee/{_cid}/totals/", {"per_page": 100})
                if _t and _t.get("results"):
                    for _row in _t["results"]:
                        _cyc = _row.get("cycle")
                        if _cyc and _cyc in active_cycles:
                            totals_sum += _row.get("receipts") or 0
                time.sleep(0.1)
            if totals_sum > 0:
                receipts_total = round(totals_sum)
