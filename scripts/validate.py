...
def load_trackaipac(path="data/raw/trackaipac.json"):
    ...
    Falls back to the legacy trackaipac_page.txt line format (PACs only) if
    the JSON is absent, for backward compatibility.
    """
    data = {}
    p = Path(path)
...
def check_aipac_vs_fec_pacs(d):
    """[S1] AIPAC_DATA (the inline-displayed PAC totals on cards) should
    approximately equal fec.aipac_pacs. Drift means one source updated
    without the other — flag for review, don't block deploy."""
    print("\n[S1] AIPAC_DATA vs fec.aipac_pacs (cross-source)...")
    mismatches = 0
    for name, aipac_val in d["aipac"].items():
        fec_pacs = d["fec_v8"].get(name, {}).get("aipac_pacs", 0) or 0
        if abs(aipac_val - fec_pacs) > 1000:
            warn(f"AIPAC drift — {name}: AIPAC_DATA=${aipac_val:,} / fec.aipac_pacs=${fec_pacs:,}")
            mismatches += 1
    if mismatches == 0:
