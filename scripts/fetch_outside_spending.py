...
def fetch_member(rec, api_key, cycles):
    ...
    sup_all, opp_all = {}, {}
    for cid in cand_ids:
        s, o, cyc = candidate_totals(cid, api_key, cycles)
        total_s += s
        total_o += o
        cycles_found |= cyc
        sup, opp = candidate_top_spenders(cid, api_key, cycles)
...
def main():
    ...
    for i, name in enumerate(members, 1):
        ...
        print(f"[{i:4d}/{len(members)}] {name:<34} "
              ...
              f"cycles={result['cycles']}  (searched {cyc_list[0]}-{cyc_list[-1]})", flush=True)
