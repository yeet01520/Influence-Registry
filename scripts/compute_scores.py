...
def funding_score(fec_record):
    """
    How much of a member's CONTRIBUTIONS came from organised money.

        organised = pac_total + party_total
        denom     = individual_total + pac_total + party_total
        score     = organised / denom * 150, capped at 100
    ...
    if denom <= 0:
        return None
    return int(round(min((pac + party) / denom * 150, 100)))
