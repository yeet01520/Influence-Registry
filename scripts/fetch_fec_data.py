...
def fetch_all_receipts(committee_ids, active_cycles, member_name):
    ...
    for cmte in committee_ids:
        # ── 1. Official per-cycle totals: total_raised + unitemized ─────
        cmte_cycles = set()
        t = get(f"/committee/{cmte}/totals/", {"per_page": 100})
        if t and t.get("results"):
            for row in t["results"]:
                cyc = row.get("cycle")
                if not cyc or cyc not in active:
                    continue
                receipts = row.get("receipts") or 0
                if receipts:
                    cmte_cycles.add(cyc)
                total_raised += receipts
                # NOTE: do NOT add individual_unitemized_contributions here.
                # The by_size size=0 bucket below already includes unitemized
                # money in its $200-and-under total; adding it from the
                # totals endpoint as well double counts (symptom: grassroots
                # jumping by exactly 2x unitemized between runs).
        time.sleep(0.1)

        # Only touch cycles where this committee actually had receipts
        for cycle in sorted(cmte_cycles, reverse=True):
            # ── 2. Itemized <=$200 bucket (FEC-computed, deduped) ───────
            ...
            # ── 3. PAC receipts only (is_individual=false) ──────────────
            last_indexes = None
            while True:
                params = {
                    "committee_id": cmte,
                ...
                for item in data["results"]:
                    ...
                    if amount <= 0:
                        continue
                    # skip conduit lumps (per-donor detail is individual-side)
                    _contrib_l = (item.get("contributor_name") or "").lower()
                    _is_memo = (item.get("memo_code") or "").upper() == "X"
                    if (not _is_memo) and any(x in _contrib_l for x in (
                            "actblue", "winred", "democracy engine", "anedot")):
                        continue

                    matched = None
                    src_cmte = (item.get("contributor_committee_id") or "").strip()
                    if src_cmte and src_cmte in SECTOR_CMTE_IDS:
                        matched = SECTOR_CMTE_IDS[src_cmte]
                    else:
                        blob = " ".join(filter(None, (
                            item.get("contributor_name") or "",
                            item.get("contributor_employer") or "",
                            item.get("contributor_occupation") or "",
                        ))).lower()
