...
def search_candidates(member_name, office, state, api_key):
    """
    Query FEC `/candidates/` endpoint for candidates matching name + office + state.
    
    Uses last-name substring search via the `name=` parameter, which is more
    reliable than `/candidates/search/?q=` because the search endpoint expects
    partial typeahead-style queries and doesn't reliably match full names.
    
    Returns list of candidate dicts.
    """
    # Strip suffixes that would mess up last-name extraction
    ...
    # Use the LAST word as last name. For compound surnames like "Van Hollen"
    # or "Ocasio-Cortez", FEC stores them with hyphens and spaces;
    # substring match on the last token still works because FEC's name= field
    # is substring-matched against the full name.
    last_name = parts[-1]
    ...
    data = fec_get("/candidates/", params, api_key)
...
def find_correct_candidate_ids(name, office, state, api_key):
    ...
    """
    # First try with state filter (server-side)
    candidates = search_candidates(name, office, state, api_key)
    api_returned_with_state = len(candidates)
    ...
    for c in candidates:
        ...
        if names_match(name, c_name):
            matching.append(c)
    name_matched_with_state = len(matching)
    
    # If no name matches with state filter, try without state to see if member
    # might be in a different state (rare — would indicate roster mismatch)
    api_returned_no_state = 0
    if not matching:
        time.sleep(SLEEP_BETWEEN_CALLS)
        candidates_all = search_candidates(name, office, "", api_key)
        api_returned_no_state = len(candidates_all)
        for c in candidates_all:
            c_name = c.get("name", "")
            if names_match(name, c_name):
                # Only accept if the candidate's state field actually matches
                if c.get("state", "").upper() == state.upper():
                    matching.append(c)
    
    if not matching:
        # Diagnostic logging — helps debug why a member wasn't found
        print(f"      no match: api_with_state={api_returned_with_state}, "
              f"api_no_state={api_returned_no_state}", flush=True)
        return None, []
    
    # For each match, accept the candidate ID and track most-recent cycle
    seen_ids = {}
    for c in matching:
        cid = c.get("candidate_id")
        if not cid:
            continue
        # Verify state via API's state field (canonical)
        c_state = (c.get("state") or "").upper()
        if c_state and c_state != state.upper():
            continue
        # Verify office via API's office field
        c_office = (c.get("office") or "").upper()
        if c_office and c_office != office.upper():
            continue
