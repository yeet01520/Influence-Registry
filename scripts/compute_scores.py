...
def calc_score(name: str, profile: dict, merged_donations: dict,
    ...
    if any_corporate:
        ...
        if aipac_amt > 0:
            score += sector_points(aipac_amt)
            reasons.append("AIPAC")

        if not explicitly_clean:
            if oil_gas > 0:
                score += sector_points(oil_gas); reasons.append("Oil/Gas")
            if pharma > 0:
                score += sector_points(pharma);  reasons.append("Pharma")
            if wall_st > 0:
                score += sector_points(wall_st); reasons.append("Finance")
            if defense > 0:
                score += sector_points(defense); reasons.append("Defense")
            if tech > 0:
                score += sector_points(tech);    reasons.append("Tech")
            # Corporate total tier — mirror the JS three-tier conversion exactly
            if corp_total > 50_000_000:
                score += sector_points(3_000_000)
            elif corp_total > 20_000_000:
                score += sector_points(1_000_000)
            elif corp_total > 5_000_000:
                score += sector_points(500_001)

        # Cap at 94
        score = min(score, 94)
        col = "#b91c1c" if score >= 70 else "#ea580c" if score >= 55 else "#d97706"
        lbl = "High Risk" if score >= 70 else "Moderate Risk" if score >= 55 else "Some Corporate"
        return {
            "pct": score, "lbl": lbl, "vto": False, "col": col,
            "super_pac_total": int(super_pac_total),
            "any_corporate": True, "reasons": reasons,
        }

    # Branch 3b: Clean / Low Risk / Moderate (no corporate money)
    base = profile.get("corruption_score") or 15
    col = "#16a34a" if base < 20 else "#65a30d" if base < 35 else "#ca8a04"
    lbl = "Clean" if base < 20 else "Low Risk" if base < 35 else "Moderate"
    return {
        "pct": base, "lbl": lbl, "vto": False, "col": col,
