...
def calc_score(name: str, profile: dict, merged_donations: dict,
    ...
    # Branch 0b: Challengers. If a manual baseline corruption_score is set
    # (brand-new candidate with no donation history to score), use it. If not,
    # the challenger has a real record (e.g. a sitting politician running for a
    # different seat) — fall through to normal scoring below.
    if name in CHALLENGER_NAMES and profile.get("corruption_score") is not None:
        cb = profile["corruption_score"]
        col = "#16a34a" if cb < 20 else "#65a30d" if cb < 35 else "#ca8a04" if cb <= 65 else "#b91c1c"
        lbl = "Clean" if cb < 20 else "Low Risk" if cb < 35 else "Moderate" if cb <= 65 else "High Risk"
        return {
            "pct": cb, "lbl": lbl, "vto": False, "col": col,
            "super_pac_total": 0, "any_corporate": False, "reasons": [],
            "score_basis": "Manual baseline — challenger with no voting record.",
        }
    ...
    #   non_individual = (total_raised - grassroots) + AIPAC
